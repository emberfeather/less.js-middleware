...
module.exports = less.middleware = function(source, options, parserOptions, compilerOptions){
      ...
      try {
        ...
        if( options.compiler.sourceMap ){
          ...
          options.compiler.writeSourceMap = function( sourceMap ){
            var mapPath = path.join( options.compiler.sourceMapBasepath, options.compiler.sourceMapURL );
            fs.writeFile( mapPath, sourceMap, function( err ){
                log( 'sourcemap', 'write sourcemap to ' + mapPath + ' error:', err );
            });
          };
        }
  ...
  return function(req, res, next) {
    ...
    if (utilities.isValidPath(pathname)) {
      ...
      // If it's source-map, just translate it to a .css request ( which will update the source-map either ).
      if( isSourceMap ){
