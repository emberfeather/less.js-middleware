...
module.exports = less.middleware = function(options){
  ...
  //pass metadata to the template and render with underscore
  ...
  return function(req, res, next) {
    ...
    if (regex.handle.test(pathname)) {
      ...
      // Parse metadata
      var parseMetadata = function(meta) {
        var _root = path.dirname(require.main.filename);
        var metadata = {};

        if (meta instanceof Array) {
          meta.forEach(function(file) {
            fill(metadata, file);
          });
        ...
        function fill(obj, file) {
          var ext = path.extname(file);
          var name = path.basename(file, ext);
          var content = require(path.join(_root, file));
          ...
          try {
            options.render(str, lessPath, cssPath, function(err, css){
              ...
              mkdirp(path.dirname(cssPath), 511 /* 0777 */, function(err){
                ...
                if (options.metadata)
                  css = _.template(css, parseMetadata(options.metadata));
