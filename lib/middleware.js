...
module.exports = less.middleware = function(options){
  ...
  return function(req, res, next) {
    ...
    if(options.preprocessPath) {
      pathname = options.preprocessPath(pathname);
    }
          ...
          try {
            ...
            options.render(preprocessed, lessPath, cssPath, function(err, css){
              ...
              mkdirp(path.dirname(cssPath), 511 /* 0777 */, function(err){
                if (err) return error(err);
                var postprocessor = options.postprocessor || function(cssPath, css, next) {
                  next(cssPath, css);
                };
                var writeToDisk = function(cssPath, css) {
                  fs.writeFile(cssPath, css, 'utf8', next);
                };
                postprocessor(cssPath, css, writeToDisk);
              });
            });
          } catch (err) {
            lessError(err);
