...
module.exports = less.middleware = function(src, options, parserOptions, compilerOptions){
  ...
  return function(req, res, next) {
    ...
    if (utilities.isValidPath(pathname)) {
      ...
      if (options.root) {
        pathname = pathname.replace(dest, '');
        cssPath = path.join(options.pathRoot, dest, pathname);
        lessPath = path.join(options.pathRoot, src, utilities.maybeCompressedSource(pathname));
      }
