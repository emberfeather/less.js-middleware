...
module.exports = less.middleware = function(options){
  ...
  if (options.sourceMap) options.dumpLineNumbers = 'comments';
  ...
  return function(req, res, next) {
    ...
    var isMap = false;
    if (options.sourceMap) pathname = req.url.replace(/\.map$/i, function () {
      isMap = true;
    ...
    if (regex.handle.test(pathname)) {
      ...
      var dir = (root) ? path.join(root, dest) : dest;
      var addPrefix = function(filePath) {
        if (prefix) filePath = path.join(prefix, filePath);
        if (filePath[0] != '/') filePath = '/' + filePath;
	return filePath;
      }
      var removeDir = function(filePath) {
        if (filePath.indexOf(dir) === 0) filePath = filePath.substring(dir.length);
        return addPrefix(filePath);
      }
      var outputSourceMap = function (css) {
        var lines = css.split('\n');
        var smg = new SourceMapGenerator({ file: addPrefix(pathname) });
      ...
      var compile = function() {
        ...
        fs.readFile(lessPath, 'utf8', function(err, str){
	      ...
	      if (options.sourceMap) {
                css += '\n/*@ sourceMappingURL=' + addPrefix(pathname) + '.map */'
	      }
              mkdirp(path.dirname(cssPath), 0777, function(err){
                if (err) return error(err);
		var noop = function () {};
                fs.writeFile(cssPath, css, 'utf8', isMap ? noop : next);
                fs.writeFile(cssPath + '.map', outputSourceMap(css), 'utf8', isMap ? next : noop);
              });
            });
          } catch (err) {
            lessError(err);
